...
module Pinion
{
    ...
    class Interface extends Ble.BleDelegate
    {
        ...
        public function initialize()
        {
            ...
            {
                Debug.log("Interface::initialize failed " + e);
            }
        }
        ...
        public function connect(deviceHandle as DeviceHandle) as Lang.Boolean
        {
            ...
            {
                Debug.log("Already connected to a device");
                return false;
            }

            if(deviceHandle.scanResult() == null)
            {
                 Debug.log("DeviceHandle ScanResult is null");
                return false;
            }
            ...
            try
            {
                ...
                if(_connectedDevice == null)
                {
                    Debug.log("Ble.pairDevice returned null");
                    return false;
                }
            }
            catch(e instanceof Ble.DevicePairException)
            {
                ...
                Debug.log("Ble.DevicePairException exception " + e.getErrorMessage());
                return false;
            }
        ...
        public function getActiveErrors() as Void
        {
            if(_connectedDevice == null)
            {
                Debug.log("Can't getActiveErrors when not connected");
                return;
            }
        ...
        public function onCharacteristicWrite(characteristic as Ble.Characteristic, status as Ble.Status) as Void
        {
            ...
            {
                Debug.log("Error writing to characteristic");
                disconnect();
                return;
            }
            ...
            {
                Debug.log("onCharacteristicWrite with no active request");
                disconnect();
                return;
            }
        ...
        public function onDescriptorWrite(descriptor as Ble.Descriptor, status as Ble.Status) as Void
        {
            if(status != Ble.STATUS_SUCCESS)
            {
                Debug.log("Error writing to descriptor");
                disconnect();
                return;
            }
            ...
            if(_currentRequest == null)
            {
                Debug.log("onDescriptorWrite with no active request");
                disconnect();
                return;
            }
            ...
            if(!success)
            {
                Debug.log("Request onDescriptorWrite failed");
                disconnect();
                return;
            }
        ...
        public function onCharacteristicChanged(characteristic as Ble.Characteristic, value as Lang.ByteArray) as Void
        {
            ...
            case PINION_CHAR_RESPONSE_UUID:
            {
                if(_currentRequest == null)
                {
                    Debug.log("Response characteristic changed with no active request");
                    disconnect();
                    return;
                }
        ...
        public function onParameterRead(parameter as ParameterType, value as Lang.Number) as Void
        {
            if(_scanState == SCANNING)
            {
                ...
                {
                    Debug.log("Last scan result is null");
                    disconnect();
                    return;
                }

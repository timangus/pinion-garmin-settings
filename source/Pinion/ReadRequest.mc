...
module Pinion
{
    class ReadRequest extends Request
    {
        ...
        public function decodeResponse(bytes as Lang.ByteArray) as Request.ResponseResult
        {
            ...
            {
                Debug.log("ReadRequest response error " + bytesToHex(bytes));
                return RESPONSE_FAILURE;
            }

            if(bytes[0] != PINION_REPLY)
            {
                Debug.log("ReadRequest response is not a reply");
                return RESPONSE_FAILURE;
            }
            ...
            {
                Debug.log("ReadRequest response is for the wrong address");
                return RESPONSE_FAILURE;
            }
